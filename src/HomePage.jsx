...
function HomePage() {
  ...
  const handleButtonClick = (event, action) => {
    ...
    if (action === 'eventDetails') {
      navigate('/event-details'); // <-- this will trigger navigation
    } else if (action === 'apply') {
      navigate('/apply'); // optional: if you want navigation for apply
    } else if (action === 'home') {
      navigate('/');
    } else if (action === 'shastra') {
      navigate('https://shastra.org'); // or open in new tab using window.open
    }
  };
  ...
  return (
    <>
         {/* Sticky Nav */}
            ...
            <div
              style={{ backgroundColor: "#FF004E" }}
              className="w-full flex justify-between items-start lg-flex-row sm-p-2"
            >
              {/* Left box */}
